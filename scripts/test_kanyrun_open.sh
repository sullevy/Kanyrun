...
run_copy_shortcut_uri_case() {
    ...
    output="$(PATH="$tmpdir:$PATH" XDG_DATA_HOME="$tmpdir/xdg" KANYRUN_TEST_COPYQ_LOG="$copyq_log" KANYRUN_TEST_URI="$uri_file" KANYRUN_TEST_PLAIN="$plain_file" KANYRUN_DISMISS_EXISTING=0 KANYRUN_OPEN_TIMING=1 KANYRUN_OPEN_TIMING_LOG="$timing_log" "$SCRIPT")"
...
run_context_menu_fallback_case() {
    ...
    set +e
    output="$(PATH="$tmpdir:$PATH" XDG_DATA_HOME="$tmpdir/xdg" KANYRUN_TEST_COPYQ_LOG="$copyq_log" KANYRUN_TEST_URI="$uri_file" KANYRUN_TEST_PLAIN="$plain_file" KANYRUN_DISMISS_EXISTING=0 KANYRUN_OPEN_TIMING=1 KANYRUN_OPEN_TIMING_LOG="$timing_log" "$SCRIPT" 2>&1)"
    status=$?
    set -e

    [ "$status" -eq 0 ] || fail 'missing paths did not fall back to default menu'
    grep -F -- "ARGS: <--daemon-idle-timeout> <300> <--ui-idle-timeout> <15> <--menu> <--from-primary>" <<< "$output" >/dev/null || fail 'context menu fallback arguments mismatch'
